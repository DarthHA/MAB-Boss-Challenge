...
{
    public class DSRay : ModProjectile
    {
        ...
        {
            if(!Main.projectile[(int)projectile.ai[0]].active || Main.projectile[(int)projectile.ai[0]].type != ModContent.ProjectileType<KyberCrystal>())
            {
                projectile.Kill();
                return;
            }
