...
{
    public class KyberCrystal2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {   
                ...
                {
                    if (Laser.active && Laser.type == ModContent.ProjectileType<DSRay2>() && Laser.ai[0] == projectile.whoAmI)
                    {
                        Laser.ai[1] = 1;
                    }
                }
                foreach (Projectile Laser in Main.projectile)
                {
                    if (Laser.active && Laser.type == ModContent.ProjectileType<DSRay3>() && Laser.ai[0] == projectile.whoAmI)
                    {
                        Laser.ai[1] = 1;
                    }
                }
            ...
            if (projectile.localAI[0] == 70 && projectile.ai[1] == 0)
            {
                ...
                int protmp = Projectile.NewProjectile(projectile.Center, projectile.velocity, ModContent.ProjectileType<DSRay2>(), (int)(projectile.damage * 1.25f), 0, default, projectile.whoAmI, 0);
                Main.projectile[protmp].rotation = -MathHelper.Pi / 2;

                protmp = Projectile.NewProjectile(projectile.Center, projectile.velocity, ModContent.ProjectileType<DSRay3>(), (int)(projectile.damage * 1.25f), 0, default, projectile.whoAmI, 0);
                Main.projectile[protmp].rotation = -MathHelper.Pi / 2;
            }
        }
