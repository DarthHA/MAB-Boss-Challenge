...
{
    public class WarpArena3 : ModProjectile
    {
        ...
        public void Drag()
        {
            ...
            {
                if (player.Distance(projectile.Center) > 720 && !player.dead && player.active)
                {
                    ...
                    if (player.Distance(projectile.Center) > 710)
                    {
                        player.Center = projectile.Center + Vector2.Normalize(player.Center - projectile.Center) * 710;
                    }
                }
            }
        }
        ...
        {
            float r = projectile.ai[1] * 18f;
            if (projectile.ai[1] < 40 && projectile.ai[1] >= 0)
            {
                r = projectile.ai[1] * 18f;
            }
            if (projectile.ai[1] >= 40) r = 720;
            if (projectile.ai[1] < 0)
            {
                r = (-projectile.ai[1] - 1) * 18;
            }
        ...
        public bool ShouldAlive()
        {
            ...
            {
                if (proj.active && proj.type == ModContent.ProjectileType<KyberCrystal2>())
                {
                    if (proj.ai[1] == 0)
                    {
                        return true;
