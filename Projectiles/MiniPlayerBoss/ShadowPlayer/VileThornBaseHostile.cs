...
{
    public class VileThornBaseHostile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] == 0f)
            {
                ...
                if (projectile.alpha <= 0)
                {
                    ...
                    if (projectile.type == ModContent.ProjectileType<VileThornBaseHostile>())
                    {
                        int num47 = projectile.type;
                        if (projectile.ai[1] >= 20f)
                        {
                            num47 = ModContent.ProjectileType<VileThornTipHostile>();
                        }
                        int num48 = Projectile.NewProjectile(projectile.Center + Vector2.Normalize(projectile.velocity) * 32, projectile.velocity, num47, projectile.damage, projectile.knockBack, projectile.owner, 0f, 0f);
                        Main.projectile[num48].damage = projectile.damage;
                        Main.projectile[num48].ai[1] = projectile.ai[1] + 1f;
                        NetMessage.SendData(MessageID.SyncProjectile, -1, -1, null, num48, 0f, 0f, 0f, 0, 0, 0);
                        return;
                    }
