...
{
    public class HotLaserHostile : ModProjectile
    {
            ...
            if (CanDamage())
            {
                foreach(Player player in Main.player)
                {
                    if(player.active && !player.dead)
                    {
                        ...
                        float point = 0f;
                        if (Collision.CheckAABBvLineCollision(player.position, player.Size, projectile.Center,
                            projectile.Center + unit * Length, 20, ref point) || Collision.CheckAABBvLineCollision(player.position, player.Size, projectile.Center,
                            projectile.Center - unit * Length, 20, ref point))
                        {
                            if (player.statLife > 3)
                            {
                                player.statLife -= 3;
                                CombatText.NewText(player.Hitbox, CombatText.DamagedHostile, 3);
                            }
                        }
                    }
                }
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            ...
            target.AddBuff(BuffID.Burning, 20);
        }
