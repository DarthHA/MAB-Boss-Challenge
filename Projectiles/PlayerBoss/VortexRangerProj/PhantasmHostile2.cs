...
{
    public class PhantasmHostile2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] % 40 == 39)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    Projectile.NewProjectile(projectile.Center + Facing * 25, (projectile.rotation + Main.rand.NextFloat() * MathHelper.Pi / 9 - MathHelper.Pi / 18).ToRotationVector2() * (8 + Main.rand.Next(6)) + Main.player[owner.target].velocity * 0.2f, ModContent.ProjectileType<LunarArrowHostile>(), projectile.damage, projectile.knockBack, Main.myPlayer);
                }
            }
            if (projectile.ai[1] % 8 == 1)
            {
                Projectile.NewProjectile(projectile.Center + Facing * 20, (projectile.rotation + MathHelper.Pi / 9).ToRotationVector2() * 30, ModContent.ProjectileType<PArrowHostile>(), projectile.damage, projectile.knockBack, Main.myPlayer);
                Projectile.NewProjectile(projectile.Center + Facing * 20, (projectile.rotation - MathHelper.Pi / 9).ToRotationVector2() * 30, ModContent.ProjectileType<PArrowHostile>(), projectile.damage, projectile.knockBack, Main.myPlayer);
            }
        }
