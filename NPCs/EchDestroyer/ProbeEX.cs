...
namespace MABBossChallenge.NPCs.EchDestroyer
{
    public class ProbeEX : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            bool JH = false;
            if (npc.justHit && npc.knockBackResist != 0) { JH = true; }
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && npc.localAI[0] >= 120f)
            {
                ...
                if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    ...
                    {
                        Vector2 ShootVel = new Vector2(num4, num5);
                        float ShootVelR = (float)Math.Atan2(ShootVel.Y, ShootVel.X);
                        protmp = Projectile.NewProjectile(vector, (ShootVelR + MathHelper.Pi / 12).ToRotationVector2() * ShootVel.Length(), num9, num8, 0f, Main.myPlayer, 0f, 0f);
                        Main.projectile[protmp].tileCollide = false;
                        protmp = Projectile.NewProjectile(vector, (ShootVelR - MathHelper.Pi / 12).ToRotationVector2() * ShootVel.Length(), num9, num8, 0f, Main.myPlayer, 0f, 0f);
                        Main.projectile[protmp].tileCollide = false;
                    }
            ...
            if (((npc.velocity.X > 0f && npc.oldVelocity.X < 0f) || (npc.velocity.X < 0f && npc.oldVelocity.X > 0f) || (npc.velocity.Y > 0f && npc.oldVelocity.Y < 0f) || (npc.velocity.Y < 0f && npc.oldVelocity.Y > 0f)) && !JH)
            {
                npc.netUpdate = true;
            }
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            Color alpha = base.npc.GetAlpha(drawColor);
            ...
            {
                Color color2 = base.npc.GetAlpha(color);
                float num7 = num2 - num6;
                if (num3 < 0)
                {
                    num7 = num4 - num6;
                }
                color2 *= num7 / (NPCID.Sets.TrailCacheLength[base.npc.type] * 1.5f);
                Vector2 value = npc.oldPos[num6];
                float rotation = base.npc.rotation;
                Main.spriteBatch.Draw(texture2D, value + base.npc.Size / 2f - Main.screenPosition + new Vector2(0f, base.npc.gfxOffY), new Rectangle?(base.npc.frame), color2, rotation + base.npc.rotation * num5 * (num6 - 1) * -spriteEffects.HasFlag(SpriteEffects.FlipHorizontally).ToDirectionInt(), origin, base.npc.scale, spriteEffects, 0f);
                num6 += num3;
            }
            SpriteEffects effects = (base.npc.direction == -1) ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
            spriteBatch.Draw(texture2D, base.npc.Center - Main.screenPosition + new Vector2(0f, base.npc.gfxOffY), new Rectangle?(base.npc.frame), alpha, base.npc.rotation, base.npc.frame.Size() / 2f, base.npc.scale, effects, 0f);
            spriteBatch.Draw(ProbeTexture, base.npc.Center - Main.screenPosition + new Vector2(0f, base.npc.gfxOffY), new Rectangle?(base.npc.frame), new Color(255, 255, 255, 0), base.npc.rotation, base.npc.frame.Size() / 2f, base.npc.scale, effects, 0f);
            return false;
        }
        ...
        {
            if (npc.ai[1] > 0 && npc.ai[1] < 150) damage /= 2;
        }
